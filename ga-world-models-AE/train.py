...
class RolloutGenerator(object):
    """ ロールアウトジェネレーター
    :attr device: AE、MDRNN、コントローラーの実行に使用されるデバイス
    :attr time_limit: ロールアウトは最大time_limitタイムステップを持つ
    """

    def __init__(self, device, time_limit, discrete_AE):
        """ ae、rnn、コントローラ、環境を構築 """
        ASIZE, LSIZE, RSIZE, RED_SIZE, SIZE = 3, 32, 256, 64, 64
        ...
        self.discrete_AE = discrete_AE # Discrete ae

        #表現が離散的であるため、潜在ベクトルのサイズを大きく
        if (self.discrete_AE):
            LSIZE = 128

        self.ae = AE(3, LSIZE, 16*32*32) # AE：3チャンネル、潜在ベクトルサイズ、中間層サイズ
    ...
    def get_action_and_transition(self, obs, hidden):
        ...
        AEを用いて観測値を潜在状態に変換し、MDRNNを用いて次の潜在状態と次の隠れ状態の推定を行い、コントローラに対応するアクションを計算する。
        ...
        """
        _, latent_mu = self.ae(obs) # latent_mu: 潜在ベクトルの期待値

        # Discrete AEの場合、離散化する
        if (self.discrete_AE):  
...
class GAIndividual():
    ...
    '''
    def __init__(self, device, time_limit, setting, multi=True, discrete_AE = False):
        '''
        コンストラクタ―
        '''
        ...
        self.discrete_AE = discrete_AE # Discrete AEのオン／オフ

        self.mutation_power = 0.01 # 突然変異時の変化量
            
        self.setting = setting # 0: MUT-ALL, 1: MUT-MOD

        self.r_gen = RolloutGenerator(device, time_limit, discrete_AE) # ロールアウトジェネレーター
        #self.r_gen.discrete_AE = self.discrete_AE
    ...
    def load_solution(self, filename):
        ...
        self.r_gen.ae.load_state_dict( s['ae']) # aeに個体のaeのパラメータをロード
    ...
    def clone_individual(self):
        ...
        個体を複製する
        """
        child_solution = GAIndividual(self.device, self.time_limit, self.setting, multi=True, discrete_AE = self.discrete_AE) # 個体クラスを生成
        ...
        child_solution.r_gen.ae = copy.deepcopy (self.r_gen.ae) # aeをコピー
    ...
    def mutate(self):
        ...
        突然変異を行う
        """
        if self.setting == 0: #Mutate controller, AE and MDRNN. Normal deep neuroevolution

            # すべてのモジュールのパラメータを突然変異させる
            self.mutate_params(self.r_gen.controller.state_dict())
            self.mutate_params(self.r_gen.ae.state_dict())
            self.mutate_params(self.r_gen.mdrnn.state_dict())

        if self.setting == 1: #Mutate controller, AE or mdrnn

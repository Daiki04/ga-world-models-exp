...
class GA:
    def __init__(self, elite_evals, top, threads, timelimit, pop_size, setting, discrete_AE):
        '''
        コンストラクタ―
        ...
        for i in range(pop_size):
            self.P.append(GAIndividual('cpu', timelimit, setting, multi= multi_process, discrete_AE=discrete_AE ) )
    ...
    def run(self, max_generations, filename, folder):
        ...
        if exists( pop_name ):
            ...
            idx = 0
            for s in pop_tmp:
                 P[idx].r_gen.ae.load_state_dict ( s['ae'].copy() ) # aeのパラメータをロード
        ...
        while (True): 
            ...
            for k in range(self.top):
                ...
                if f > max_fitness: #best fitness ever found
                    ...
                    torch.save({'ae': elite.r_gen.ae.state_dict(), 'controller': elite.r_gen.controller.state_dict(), 'mdrnn':elite.r_gen.mdrnn.state_dict(), 'fitness':f}, "{0}/best_{1}G{2}.p".format(folder, filename, i))
            ...
            for s in P:
                 ...
                 save_pop += [{'ae': s.r_gen.ae.state_dict(), 'controller': s.r_gen.controller.state_dict(), 'mdrnn':s.r_gen.mdrnn.state_dict(), 'fitness':fitness, 'generation':i}]

...
class RolloutGenerator(object):
    ...
    def get_action_and_transition(self, obs, hidden, latent_mu_prediction):
        ...
        if latent_mu_prediction is None:
            action = self.controller(latent_mu, hidden[0] ) # コントローラーによるアクションの計算
        else:
            action = self.controller(latent_mu_prediction, hidden[0] ) # コントローラーによるアクションの計算

        mus, sigmas, logpi, rs, d, next_hidden = self.mdrnn(action, latent_mu, hidden) # MDRNNによる次の潜在状態と次の隠れ状態の推定
        next_latent_mu_prediction = self.sample_normal_distribution(logpi, mus) # 潜在ベクトルの期待値のサンプリング

        return action.squeeze().cpu().numpy(), next_hidden, next_latent_mu_prediction.squeeze() # アクション、次の隠れ状態、潜在ベクトルの期待値を返す
    
    def sample_normal_distribution(self, logpi_nlat, mu_nlat):
        """
        logpi_nlatから各Batchごとに確率piを計算し、
        各Batchごとの確率piに基づいてmu_nlatのN_GAUSS個の正規分布から1つ正規分布を選択する関数。

        Args:
            logpi_nlat (torch.Tensor): (SEQ_LEN, BSIZE, N_GAUSS)の形状を持つlog(pi)のテンソル
            mu_nlat (torch.Tensor): (SEQ_LEN, BSIZE, N_GAUSS, LSIZE)の形状を持つmuのテンソル

        Returns:
            selected_mu_nlat (torch.Tensor): (SEQ_LEN, BSIZE, LSIZE)の形状を持つ選択されたmuのテンソル
        """
        SEQ_LEN, BSIZE, N_GAUSS, LSIZE = logpi_nlat.shape[0], logpi_nlat.shape[1], mu_nlat.shape[2], mu_nlat.shape[3]

        # 1. logpi_nlatから各Batchごとに確率piを計算
        pi_nlat = torch.softmax(logpi_nlat, dim=2)

        # 2. 各Batchごとの確率piに基づいて正規分布をサンプリング
        # pi_nlatを使って正規分布をサンプリング
        sampled_indices = torch.multinomial(pi_nlat.view(-1, N_GAUSS), 1).squeeze()

        # サンプリングしたインデックスを使用してmu_nlatから正規分布を選択
        # reshapeしてから選択
        sampled_indices = sampled_indices.view(SEQ_LEN, BSIZE, 1, 1).expand(-1, -1, -1, LSIZE)
        selected_mu_nlat = torch.gather(mu_nlat, 2, sampled_indices)

        # selected_mu_nlatは(SEQ_LEN, BSIZE, 1, LSIZE)の形状を持つので、不要な次元を削除
        selected_mu_nlat = selected_mu_nlat.squeeze(dim=2)

        return selected_mu_nlat
    ...
    def do_rollout(self, render=False,  early_termination=True):
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if i == 0:
                    action, hidden, next_latent_mu_prediction = self.get_action_and_transition(obs, hidden, None) # 行動を起こし、遷移：action(1, ASIZE), hidden(1, RSIZE), next_latent_mu_prediction(1, LSIZE)
                else:
                    action, hidden, next_latent_mu_prediction = self.get_action_and_transition(obs, hidden, next_latent_mu_prediction) # 行動を起こし、遷移：action(1, ASIZE), hidden(1, RSIZE), next_latent_mu_prediction(1, LSIZE)

...
class RolloutGenerator(object):
    """ ロールアウトジェネレーター
    :attr device: Dense、MDRNN、コントローラーの実行に使用されるデバイス
    :attr time_limit: ロールアウトは最大time_limitタイムステップを持つ
    """

    def __init__(self, device, time_limit, discrete_Dense):
        """ dense、rnn、コントローラ、環境を構築 """
        ASIZE, LSIZE, RSIZE, RED_SIZE, SIZE = 3, 32, 256, 64, 64
        ...
        self.discrete_Dense = discrete_Dense # Discrete Denseのオン／オフ

        #表現が離散的であるため、潜在ベクトルのサイズを大きく
        if (self.discrete_Dense):
            LSIZE = 128

        self.dense = Dense(3, LSIZE, 1024) # Dense：3チャンネル、潜在ベクトルサイズ、中間層サイズ
    ...
    def get_action_and_transition(self, obs, hidden):
        ...
        Denseを用いて観測値を潜在状態に変換し、MDRNNを用いて次の潜在状態と次の隠れ状態の推定を行い、コントローラに対応するアクションを計算する。
        ...
        """
        latent_mu = self.dense(obs) # latent_mu: 潜在ベクトルの期待値

        # Discrete Denseの場合、離散化する
        if (self.discrete_Dense):  
...
class GAIndividual():
    ...
    '''
    def __init__(self, device, time_limit, setting, multi=True, discrete_Dense = False):
        '''
        コンストラクタ―
        '''
        ...
        self.r_gen = RolloutGenerator(device, time_limit, discrete_Dense) # ロールアウトジェネレーター
        #self.r_gen.discrete_Dense = self.discrete_Dense
    ...
    def load_solution(self, filename):
        ...
        self.r_gen.dense.load_state_dict( s['dense']) # denseに個体のdenseのパラメータをロード
    ...
    def clone_individual(self):
        ...
        個体を複製する
        """
        child_solution = GAIndividual(self.device, self.time_limit, self.setting, multi=True, discrete_Dense = self.discrete_Dense) # 個体クラスを生成
        ...
        child_solution.r_gen.dense = copy.deepcopy (self.r_gen.dense) # denseをコピー
    ...
    def mutate(self):
        ...
        突然変異を行う
        """
        if self.setting == 0: #Mutate controller, Dense and MDRNN. Normal deep neuroevolution

            # すべてのモジュールのパラメータを突然変異させる
            self.mutate_params(self.r_gen.controller.state_dict())
            self.mutate_params(self.r_gen.dense.state_dict())
            self.mutate_params(self.r_gen.mdrnn.state_dict())

        if self.setting == 1: #Mutate controller, Dense or mdrnn

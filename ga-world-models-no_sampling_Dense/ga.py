...
class GA:
    def __init__(self, elite_evals, top, threads, timelimit, pop_size, setting, discrete_Dense):
        '''
        コンストラクタ―
        ...
        :args pop_size: 個体数
        :args setting: 設定
        :args discrete_Dense: 離散Denseのオン／オフ
        '''
        ...
        for i in range(pop_size):
            self.P.append(GAIndividual('cpu', timelimit, setting, multi= multi_process, discrete_Dense=discrete_Dense ) )
    ...
    def run(self, max_generations, filename, folder):
        ...
        if exists( pop_name ):
            ...
            idx = 0
            for s in pop_tmp:
                 P[idx].r_gen.dense.load_state_dict ( s['dense'].copy() ) # denseのパラメータをロード
        ...
        while (True): 
            ...
            for k in range(self.top):
                ...
                if f > max_fitness: #best fitness ever found
                    ...
                    torch.save({'dense': elite.r_gen.dense.state_dict(), 'controller': elite.r_gen.controller.state_dict(), 'mdrnn':elite.r_gen.mdrnn.state_dict(), 'fitness':f}, "{0}/best_{1}G{2}.p".format(folder, filename, i))
            ...
            for s in P:
                 ...
                 save_pop += [{'dense': s.r_gen.dense.state_dict(), 'controller': s.r_gen.controller.state_dict(), 'mdrnn':s.r_gen.mdrnn.state_dict(), 'fitness':fitness, 'generation':i}]
